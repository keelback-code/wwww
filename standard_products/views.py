...
def add_standard_product(request):
    ...
    if request.method == 'POST':
        form = PostForm(request.POST, request.FILES)  #rename form
        if form.is_valid():
            form.save()
...
def edit_standard_product(request, product_id):
    ...
    product = get_object_or_404(StandardProduct, pk=product_id)
    if request.method == 'POST':
        form = PostForm(request.POST, request.FILES, instance=product) #rename form
        if form.is_valid():  
            product = form.save()
        ...
        form = PostForm(instance=product)  #rename form
